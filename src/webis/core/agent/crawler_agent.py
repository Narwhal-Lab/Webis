...
class CrawlerAgent:
    ...
    def run(
        ...
        Args:
            ...
            exclude_urls: Set of already-seen URLs to skip (dedup across iterations)
            iteration: Current iteration index (used to vary queries)
            
        Returns:
            List of fetched documents
        """
        self._exclude_urls = exclude_urls or set()
        ...
        {{
            "plan": [
                ...
                {{ "tool": "tavily_search", "query": "DeepSeek-V3 performance review", "reason": "General web research fallback" }}
            ]
        }}
        """
        ...
        if not plan:
            # On subsequent iterations, rephrase the query to get different
            # results from search engines that previously returned duplicates.
            base_query = task
            if self._iteration == 1:
                base_query = f"{task} latest updates"
            elif self._iteration >= 2:
                base_query = f"{task} news report 2026"

            plan = [
                {"tool": "tavily_search", "query": task},
                {"tool": "bocha_search", "query": base_query},
                {"tool": "serpapi", "query": task},
        ...
        for step in plan:
            ...
            try:
                ...
                for doc in new_docs:
                    if len(all_docs) >= limit:
                        break
                    # URL-level dedup: skip docs whose URL was already seen
                    doc_url = (doc.meta.url if doc.meta else None)
                    if doc_url and doc_url in self._exclude_urls:
                        continue
                    all_docs.append(doc)
            except Exception as e:
